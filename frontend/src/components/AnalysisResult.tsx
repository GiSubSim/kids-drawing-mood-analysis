...
export default function AnalysisResult({ data, persona, files, onReset }: Props) {
  ...
  useEffect(() => {
    // 폰트 크기: PC용 / 모바일용 구분은 렌더링 시 처리하지만 기본적으로 큰 사이즈 유지
    const sizeClasses = [
      "text-[42px] md:text-[72px] font-black", 
      "text-[36px] md:text-[56px] font-black", 
      "text-[30px] md:text-[48px] font-bold",  
      "text-[24px] md:text-[40px] font-bold",  
      "text-[20px] md:text-[32px] font-bold"   
    ];
  ...
  const personaThemes: { [key: string]: any } = {
    "마음박사 페페": { main: "#E91E63", highlight: "#F8BBD0", key: "pepe", subTitle: "마음 박사 페페의" },
    "현실친구 라봉이": { main: "#FF9800", highlight: "#FFE0B2", key: "labong", subTitle: "현실친구 라봉이의" },
    "칭찬봇 피코": { main: "#4CAF50", highlight: "#C8E6C9", key: "pico", subTitle: "칭찬봇 피코의" },
    "카리스마 샤샤": { main: "#03A9F4", highlight: "#B3E5FC", key: "shasha", subTitle: "카리스마 샤샤의" },
  };

  const theme = personaThemes[persona] || personaThemes["마음박사 페페"];
  
  // 공통 텍스트 스타일: 모바일(18px) -> PC(24px)
  const commonTextStyle = "text-[18px] md:text-[24px] leading-[1.6] md:leading-[1.8] font-bold text-[#333] whitespace-pre-line break-keep";
  ...
  const chartData = [
    { subject: '즐거움', A: analysis_result.energy_chart.joyful, icon: '🌈' },
    { subject: '궁금증', A: analysis_result.energy_chart.curious, icon: '🔍' },
    { subject: '반짝임', A: analysis_result.energy_chart.sparkle, icon: '✨' },
    { subject: '휴식', A: analysis_result.energy_chart.rest, icon: '🍃' },
    { subject: '멍때림', A: analysis_result.energy_chart.spacing_out, icon: '🫧' },
  ];
  ...
  const renderCustomTick = ({ payload, x, y, textAnchor }: any) => {
    ...
    return (
      <g className="recharts-layer recharts-polar-angle-axis-tick">
        <text x={x} y={y} dy={0} textAnchor={textAnchor} fill="#333" style={{ fontSize: '14px', fontWeight: '800', fontFamily: 'var(--font-pretendard)' }}>
          {data.subject} {data.icon}
        </text>
      </g>
    );
  ...
  const getRowAlignment = (index: number) => {
    switch(index) {
        ...
        case 1: return "justify-end";   
        case 2: return "justify-center"; 
        case 3: return "justify-start pl-[10%]"; 
        case 4: return "justify-end pr-[10%]";    
        default: return "justify-start";
    }
  }
  ...
  return (
    // w-[1100px] 제거 -> w-full max-w-[1100px]
    <div className="w-full max-w-[1100px] mx-auto space-y-8 md:space-y-16 pb-24 font-sans animate-in fade-in duration-1000">
      
      {/* 1. 상단 타이틀 & 캐릭터 */}
      <section className="flex flex-col items-center pt-4 relative">
        <div className="text-center mb-4 relative z-20">
          <p className="text-[16px] md:text-[20px] font-bold text-[#666] mb-2">{theme.subTitle}</p>
          <h1 className="text-[32px] md:text-[52px] font-black text-[#111] tracking-tight text-center">마음 분석 결과</h1>
        </div>
        <div className="w-full flex justify-center items-end relative z-0">
          <div className="relative">
            {/* 캐릭터 크기 반응형 */}
            <img src={`/images/${theme.key}_half.png`} alt={persona} className="w-[200px] md:w-[420px] h-auto object-contain animate-in fade-in slide-in-from-bottom-4 duration-1000 z-10 relative" />
          </div>
        </div>
        {/* 설명 박스 패딩 반응형 */}
        <div className="w-full bg-white rounded-[30px] md:rounded-[40px] p-8 md:p-14 shadow-2xl relative z-10 -mt-8 md:-mt-16">
           <div className={`${commonTextStyle} text-center`}>{commentary_sections[0]?.content}</div>
        </div>
      </section>

      {/* 2. 그림 섹션 */}
      <section className="bg-white rounded-[30px] md:rounded-[40px] p-6 md:p-12 shadow-sm border border-white/30">
        <h3 className="text-center mb-6 md:mb-10"><span className="text-[24px] md:text-[30px] font-black" style={highlightStyle}>그림</span></h3>
        <div className="flex flex-wrap justify-center gap-2 md:gap-4"> 
          {previews.map((src, i) => (
            // 그림 크기 반응형
            <div key={i} className="w-[140px] h-[140px] md:w-[230px] md:h-[230px] rounded-[20px] md:rounded-[32px] overflow-hidden border-2 border-[#f0f0f0] bg-[#fafafa] shadow-inner flex-shrink-0">
              <img src={src} className="w-full h-full object-cover" alt="drawing" />
            </div>
          ))}
        </div>
      ...
      {/* 3. 마음 키워드 섹션 */}
      <section className="bg-white rounded-[30px] md:rounded-[40px] p-8 md:p-16 shadow-sm border border-white/30 text-center">
        <h3 className="mb-8 md:mb-14"><span className="text-[24px] md:text-[30px] font-black" style={highlightStyle}>마음 키워드</span></h3>
        <div className="w-full max-w-[850px] mx-auto flex flex-col gap-y-4 md:gap-y-8 mb-8 md:mb-12 py-4">
          {shuffledKeywords.map((item, index) => (
            <div key={item.originalIndex} className={`w-full flex items-center ${getRowAlignment(index)} animate-in fade-in slide-in-from-bottom-2 duration-700`}>
              <span 
                className={`${item.sizeClass} tracking-tighter transition-all duration-500 hover:scale-105 cursor-default leading-none drop-shadow-sm`} 
        ...
        <div className={`${commonTextStyle} px-2 md:px-10`}>
          {commentary_sections[1]?.content}
        </div>
      </section>
      ...
      {/* 4. 마음 에너지 섹션 */}
      <section className="bg-white rounded-[30px] md:rounded-[40px] p-8 md:p-16 shadow-sm border border-white/30 text-center">
        <h3 className="mb-6 md:mb-14"><span className="text-[24px] md:text-[30px] font-black" style={highlightStyle}>마음 에너지</span></h3>
        {/* 차트 높이 반응형 */}
        <div className="h-[300px] md:h-[650px] w-full mb-6 md:mb-10 -mt-4 md:-mt-10"> 
          <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="space-y-6 md:space-y-12">
          <div className="flex justify-center">
            <p className={`${commonTextStyle} text-center max-w-[700px] px-2`}>
              {formatEnergyText(analysis_result.persona_energy_sentence)}
            </p>
          </div>
          <div className="h-[2px] w-12 md:w-16 bg-gray-100 mx-auto"></div>
          <div className="flex justify-center">
            <p className={`${commonTextStyle} text-center max-w-[850px] px-2`}>
              {commentary_sections[2]?.content}
            </p>
          </div>
        </div>
      ...
      {/* 5. 그림 솔루션 섹션 */}
      <section className="bg-white rounded-[30px] md:rounded-[40px] p-8 md:p-16 shadow-sm border border-white/30 relative overflow-hidden">
        <h3 className="text-center mb-6 md:mb-8"><span className="text-[24px] md:text-[30px] font-black" style={highlightStyle}>그림 솔루션</span></h3>
        
        <p className="text-center mb-8 md:mb-16 text-[16px] md:text-[24px] leading-relaxed px-2 md:px-10" style={{ color: theme.highlight }}>
          그림 솔루션은 지금의 마음을 환기해 새롭게 바꿔보거나, <br/>좋은 감정을 더 확장해서 이어가 보는 그림 활동 가이드예요.
        </p>

        {/* 모바일: 세로 배치 (flex-col-reverse), PC: 가로 배치 (md:flex-row) */}
        <div className="flex flex-col-reverse md:flex-row gap-8 md:gap-12 items-center md:items-start mb-10 md:mb-20">
          ...
          <div className="w-[150px] md:w-[340px] flex-shrink-0 relative">
            <img src={`/images/${theme.key}_side.png`} alt="side char" className="w-full h-auto object-contain" />
          </div>
        </div>
        <div className="mt-8 md:mt-16 bg-[#FBFBFB] rounded-[24px] md:rounded-[36px] p-6 md:p-14 border-l-[8px] md:border-l-[14px] border-amber-300 text-left shadow-inner">
           <h4 className="text-amber-600 font-black text-[18px] md:text-[26px] mb-4 md:mb-8">⚠️ 결과가 생각한 것과 다른가요?</h4>
           <ul className="space-y-3 md:space-y-6 text-[14px] md:text-[22px] text-[#666] font-bold list-disc ml-6 md:ml-10 leading-relaxed">
             <li>같은 그림이라도 바라보는 마음에 따라 달라질 수 있어요.</li>
             <li>분석 결과는 정답이 아니라, 마음을 가볍고 즐겁게 이해하기 위한 작은 힌트예요.</li>
             <li>앞으로 나의 마음을 소중히 아끼고 돌보며, 더 즐겁게 자라나는 어린이가 되길 응원 할게요!</li>
           </ul>
      ...
      {/* 확인 버튼 */}
      <div className="flex justify-center pt-6 md:pt-10 pb-20">
        <button onClick={onReset} className="bg-[#5C9DFF] text-white w-full max-w-[300px] md:w-auto md:px-48 py-5 md:py-9 rounded-[32px] font-black text-[24px] md:text-[36px] hover:bg-[#4A8DFF] transition-all shadow-2xl active:scale-95">확인</button>
      </div>
    </div>
  );
}