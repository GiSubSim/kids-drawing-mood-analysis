...
* **[작성 템플릿 - 각 라인 끝에 `\\n` 필수]:**
    ...
    3.  [마음 에너지 해석] 마음을 따뜻하게 읽는다. (예: "신나게 웃고 있지만 동시에 마음이 조용히 쉬고 있는 느낌도 전해져요.\\n아마도 너의 마음이 행복하면서도 편안했던 날이었을 거예요.\\n")
...
* **[작성 템플릿 - 각 라인 끝에 `\\n` 필수]:**
    ...
    3.  [마음 에너지 해석] 마음을 쿨하게 인정한다.(예: "이런 그림은 말이 필요 없음. 보면 그냥 '아, 오늘 마음 괜찮았네' 이 소리 나와.\\n특히 차분한 색이랑 여유 있는 장면들, 그거 다 지금 네 상태 그대로야.\\n")
...
* **[작성 템플릿 - 각 라인 끝에 `\\n` 필수]:**
    ...
    3.  [마음 에너지 해석] 마음을 심층적으로 분석한다. (예: "여백과 구성에서 마음이 조급하지 않은 상태가 감지되며,\\n전체적으로 균형 잡힌 기분이 느껴집니다.\\n")
...
* **[작성 템플릿 - 각 라인 끝에 `\\n` 필수]:**
    ...
    3.  [마음 에너지 해석] 센스를 인정하고 마음에 용기를 준다.(예: "특히 ~한 이 부분(분석된 그림에서 특징적인 부분을 하나 찾아서 표현), 진짜 센스 있다.\\n나였으면 생각 못 했을 텐데, 너 관찰력이 진짜 좋은 것 같아.\\n")
